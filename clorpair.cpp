...
void Tim_dai(Point2D *C , float d, int &Mid_L , int &Mid_R , int L , int R){
	int Mid = (L + R) / 2 ;
	for( int i = L ; i <= R ; i++ )
		if( C[i].x >= C[Mid].x - d ){
			Mid_L = i ; break ;
...
bool TrongHinhVuong( Point2D a , Point2D A , float d ){
...
Point2D* distance2D_mid(Point2D *C , int L , int R , float d){
	float Min = 10000000000;
	Point2D *p = new Point2D[2];
...
Point2D* distance2D_min(Point2D *C , int L , int R){
	...
	float d = distance2D(min_left_right[0], min_left_right[1]);
...
int main(){
	...
	if(res[0].index > res[1].index){
		printf("%d %d %.6f",res[1].index, res[0].index, distance2D(res[1], res[0]));
	}else printf("%d %d %.6f",res[0].index, res[1].index, distance2D(res[1], res[0]));
